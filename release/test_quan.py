...
parser.add_argument('--test', type=str, default='/media/administrator/Document/project/svr/comparison_quanlity', help='model path')
...
test_path = os.path.join(opt.test, 'vipnet_256', opt.cat, 'thres_'+str(opt.thres))
...
with torch.no_grad():
    for it, data in enumerate(dataloader, 0):
        ...
        iou_pre = compute_iou(output_ref, vox_gt[:, :, :])
        total_n += 1
