...
parser.add_argument('--patchNum', type=int, default=16, help='input patch size')
...
parser.add_argument('--log', type=str, default='log_patch_l2h_with_pc_guidance', help='log path')
parser.add_argument('--nepoch', type=int, default=12, help='number of epochs to train for')
...
# dataset_val = ShapeNet(img_root='/data/dudong/ShapeNetCore.v1/renderingimg',
#                        vox256_root='/data/dudong/ShapeNetCore.v1/vox256',
#                        filelist_root='/data/dudong/ShapeNetCore.v1/train_val_test_list',
#                        cat='03001627', mode='val', patch_num=opt.patchNum, view_pick=True)
...
if os.path.exists(os.path.join(model_path, 'patch_l2h_with_pc_guidance.pt')):
    network_patch_l2h.load_state_dict(torch.load(os.path.join(model_path, 'patch_l2h_with_pc_guidance.pt')))
else:
    network_patch_l2h.generator.load_state_dict(torch.load(os.path.join(model_path, 'patch_l2h.pt')))
print('Loaded pre-trained patch_l2h.pt weights.')


# Create Loss Module
criterion = nn.BCEWithLogitsLoss()

# Create optimizer, just optimize the parameters of refiner
optimizer = optim.Adam(network_patch_l2h.refiner.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
...
for epoch in range(1, opt.nepoch + 1):
    ...
    for it, data in enumerate(dataloader, 0):
        ...
        _, _, vox_ref, vox_ref_sigmoid = network_patch_l2h(vox_in_cat, idx, pc, pc_occ)

        loss = criterion(vox_ref, vox64)
    ...
    #         vox64 = vox64.contiguous().view(opt.patchNum, 1, 64, 64, 64)
    #
    #         vox_pre, vox_pre_sigmoid = network_patch_l2h(vox16)
    ...
    torch.save(network_patch_l2h.state_dict(), os.path.join(model_path, 'patch_l2h_with_pc_guidance.pt'))
    print('save model')
print('Training done!')
