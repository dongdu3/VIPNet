...
parser.add_argument('--gen', type=str, default='vox256_gen', help='model path')
...
gen_path = os.path.join(opt.dataRoot, opt.gen, opt.cat)
...
dataset = ShapeNet(img_root=os.path.join(opt.dataRoot, 'renderingimg'),
                   ...
                   cat=opt.cat, mode='train', patch_num=opt.patchNum)
...
with torch.no_grad():
    for it, data in enumerate(dataloader, 0):
        ...
        vox_pre_sigmoid_np = np.array(vox_pre_sigmoid.cpu().data.squeeze().numpy(),
                                      dtype=np.float32).reshape((opt.patchNum, 64, 64, 64))
        ...
        np.save(os.path.join(gen_path, name[0] + '_' + view_id[0] + '.npy'), output)

        n_process += 1
        print('processed %d/%d: %s, %s' % (n_process, len_dataset, name[0], view_id[0]))
